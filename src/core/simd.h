...
namespace rt {
    ...
    inline void dot4(const Vec3x4& a, const Vec3x4& b, double results[4]) {
    ...
    inline void length_sq4(const Vec3x4& v, double results[4]) {
        dot4(v, v, results);
    }

    inline Vec3x4 normalize4(const Vec3x4& v) {
    ...
    struct SphereBlock {
        alignas(32) double cx[4];
        alignas(32) double cy[4];
        alignas(32) double cz[4];
        alignas(32) double r2[4];

        SphereBlock() {
            for (int i = 0; i < 4; i++) { cx[i]=cy[i]=cz[i]=0.0; r2[i]=-1.0; }
        }
    };

    inline void hit_spheres4(const Ray& ray, const SphereBlock& block, double results[4]) {
        __m256d ox = _mm256_set1_pd(ray.origin.x);
        __m256d oy = _mm256_set1_pd(ray.origin.y);
        __m256d oz = _mm256_set1_pd(ray.origin.z);
        __m256d dx = _mm256_set1_pd(ray.direction.x);
        __m256d dy = _mm256_set1_pd(ray.direction.y);
        __m256d dz = _mm256_set1_pd(ray.direction.z);

        __m256d ocx = _mm256_sub_pd(ox, _mm256_load_pd(block.cx));
        __m256d ocy = _mm256_sub_pd(oy, _mm256_load_pd(block.cy));
        __m256d ocz = _mm256_sub_pd(oz, _mm256_load_pd(block.cz));

        __m256d a = _mm256_set1_pd(dot(ray.direction, ray.direction));
        ...
        __m256d c = _mm256_sub_pd(
            ...
            _mm256_load_pd(block.r2));

        __m256d disc = _mm256_sub_pd(_mm256_mul_pd(half_b, half_b), _mm256_mul_pd(a, c));

        __m256d zero     = _mm256_setzero_pd();
        __m256d hit_mask = _mm256_cmp_pd(disc, zero, _CMP_GE_OQ);

        __m256d sqrt_disc  = _mm256_sqrt_pd(_mm256_max_pd(disc, zero));
        __m256d neg_half_b = _mm256_sub_pd(zero, half_b);
        __m256d t          = _mm256_div_pd(_mm256_sub_pd(neg_half_b, sqrt_disc), a);

        _mm256_store_pd(results, _mm256_blendv_pd(_mm256_set1_pd(-1.0), t, hit_mask));
    }

    inline void hit_sphere4(const Ray4& ray, const Sphere& sphere, double results[4]) {
