...
namespace rt {
    ...
    auto tinted_glass = [](Vec3d tint, double ior) -> Material {
        auto base = glass(ior);

        return [base, tint](const Ray& r, const HitRecord& rec)
            -> std::optional<Scatter>
        {
            auto result = base(r, rec);
            if (result)
                result->attenuation = tint;
            return result;
        };
    };
