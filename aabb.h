...
namespace rt {
    ...
    inline AABB surrounding_box(AABB a, AABB b){
        return AABB(Vec3d(std::min(a.min_pt.x, b.min_pt.x),std::min(a.min_pt.y, b.min_pt.y),std::min(a.min_pt.z, b.min_pt.z)), Vec3d(std::max(a.max_pt.x, b.max_pt.x),std::max(a.max_pt.y, b.max_pt.y),std::max(a.max_pt.z, b.max_pt.z)));
    }
}
